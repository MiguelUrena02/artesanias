...
class ArtesanosControllers{
    ...
    static async indexPost(req, res) {
        try {
            ...
            const insertedId = await ArtesanosModel.insertar(newData);

            res.status(201)
                .header('Location', `/artesanos/${insertedId}`)
    ...
    static async itemPatch(req, res) {
        try {
            ...
            const result = await ArtesanosModel.actualizar(id, updatedFields);
